  ...
  useEffect(() => {
    ...
    const fetchItemData = async () => {
      ...
      try {
        ...
        setProducts([amazonResponse.data]); // Wrap single product in an array
  
        // Fetch Jumia product data
        ...
        setJumiaProducts([jumiaResponse.data]); // Wrap single product in an array
  
      } catch (err) {
        console.error('Error fetching item data:', err.response?.data || err.message);
        setError(err.response?.data?.detail || 'An error occurred while fetching item data.');
      } finally {
        setLoading(false);
      }
    };
        ...
        return (
          <div className="p-4 bg-white rounded-lg shadow-md">
            ...
            {products.length > 0 ? (
              products.map((product) => (
                <div key={product.id} className="bg-gray-50 shadow rounded p-4 mb-4">
                  <Image src={product.image_source} alt={product.name} width={500} height={500} className="w-200 h-180 object-cover" />
                  <h3 className="text-lg font-semibold">{product.name}</h3>
                  <p className="text-gray-700">Price: <span className="font-medium">${product.price}</span></p>
                  <p className="text-gray-700">Rating: <span className="font-medium">{product.rating}</span></p>
                  <p className="text-gray-700">In Stock: <span className="font-medium">{product.in_stock}</span></p>
                  <a href={product.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline">
                    View on Amazon
                  </a>
                  <p className="text-gray-500 text-sm mt-2">Last Updated: {new Date(product.timestamp).toLocaleString()}</p>
                </div>
              ))
            ) : (
              <p>No Amazon product data available.</p>
            )}
            ...
            {jumiaProducts.length > 0 ? (
              ...
              <p>No Jumia product data available.</p>
            )}
          </div>
        );
        case 'graph':
